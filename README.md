...
```
tanzu apps workload create accuweatherapi -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/accuweatherapi-source \
  --type web
```
...
```
tanzu apps workload get accuweatherapi
```
