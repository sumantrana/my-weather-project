...
```json
{
  "applicationName" : "AccuWeather",
  "projectName" : "AccuWeatherAPI"
}
```
## Log
```
...
┃ ┃ ┃ ┗ ┗ ┗  Info Will replace [Sample->AccuWeather]
...
┃ ┃ ┃ ┗ ┗ ┗ Debug Path 'Sample.csproj' matched '^(?<folder>.*/)?(?<filename>([^/]+?|)(?=(?<ext>\.[^/.]*)?)$)' with groups {ext=.csproj, folder=null, filename=Sample.csproj, g0=Sample.csproj, g1=null, g2=Sample.csproj, g3=Sample.csproj, g4=.csproj} and was rewritten to 'AccuWeather.csproj'
...
┃ ┃ ┗ ┗ ┗ ┗  Info Will replace [sample-app->accuweatherapi]
